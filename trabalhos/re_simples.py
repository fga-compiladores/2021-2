"""
Competência re_simples
======================

Criar padrões de regex simples a partir de operações básicas. Este exercício
verifica alguns padrões típicos que aparecem no contexto de expressões regulares
do Python.

Complete o atributo regex = r"..." de cada classe com a expressão regular correta.
Você deve habilitar os testes com enable = True para cada exercício. Eles são
desabilitados por padrão para evitar mensagens de erro muito longas.

É necessário passar em todos os testes para demonstrar competência.
"""
...
class texto_que_nao_contêm_as_letras_c_o_v_i_d(Re):
    """
    bar  42  *^*#$)@$  2019 k0r0na COVID!
